...
class UserAgent
{
	 ...
	 * @var array $windowsOS
	 */
	public array $windowsOS = ['[Windows; |Windows; U; |]Windows NT 6.:number0-3:;[Win64; x64| WOW64| x64|]', '[Windows; |Windows; U; |]Windows NT 10.:number0-5:;[Win64; x64| WOW64| x64|]',];
	/**
	 * Linux Operating Systems [limited]
	 * @var array $linuxOS
	 */
	public array $linuxOS = ['[Linux; |][U; |]Linux x86_64', '[Linux; |][U; |]Linux i:number5-6::number4-8::number0-6: [x86_64|]'];
	/**
	 * Mac Operating System (OS X) with dynamic versioning
	 * @var array $macOS
	 */
	public array $macOS = ['Macintosh; [U; |]Intel Mac OS X :number7-9:_:number0-9:_:number0-9:', 'Macintosh; [U; |]Intel Mac OS X 10_:number0-12:_:number0-9:'];
	/**
	 * Versions of Android to be used
	 * @var array $androidVersions
	 */
	public array $androidVersions = ['4.3.1', '4.4', '4.4.1', '4.4.4', '5.0', '5.0.1', '5.0.2', '5.1', '5.1.1', '6.0', '6.0.1', '7.0', '7.1', '7.1.1'];
	/**
	...
	public function getOS(string $os = null): string
	{
		...
		if($os === null || in_array($os, ['chrome', 'firefox', 'explorer'])) {
			$oses = $os === 'explorer' ? $this->windowsOS : array_merge($this->windowsOS, $this->linuxOS, $this->macOS);
		} else {
			$oses += $this->{$os.'_os'};
		}
		// randomly select on operating system
		$selected_os = rtrim($oses[random_int(0, count($oses) - 1)], ';');
	...
	/**
	 * Get Mobile OS
	 * @param string|null $os Can specify android, iphone, ipad, ipod, or null/blank for random
	 * @return array|string *
	 * @throws \Random\RandomException
	 */
	public function getMobileOS(string $os = null): array|string
	{
		$os = strtolower($os);
		$oses = match ($os) {
			'android' => $this->androidOS,
			'iphone', 'ipad', 'ipod' => [$this->mobileIOS[$os]],
			default => array_merge($this->androidOS, array_values($this->mobileIOS)),
		};

		// select random mobile os
		$selected_os = rtrim($oses[random_int(0, count($oses) - 1)], ';');
		if(str_contains($selected_os, ':androidVersion:')) {
			$selected_os = $this->processAndroidVers($selected_os);
		}
	 ...
	 * @return null|string|string[] *
	 */
	public function processAndroidVers($selected_os): array|string|null
	{
	 ...
	 * @throws Exception
	 */
	public function generate($user_agent = null): string
	{
		if($user_agent === null) {
			$rand = random_int(0, 100);
			$user_agent = ($rand >= 44) ? array_rand(['firefox' => 1, 'chrome' => 1, 'explorer' => 1]) : array_rand(['iphone' => 1, 'android' => 1, 'mobile' => 1]);
		} elseif(in_array($user_agent, ['windows', 'mac', 'linux'])) {
			$agents = ['firefox' => 1, 'chrome' => 1];
			if($user_agent == 'windows') {
				$agents['explorer'] = 1;
			}
			$user_agent = array_rand($agents);
		}
		$_SESSION['agent'] = $user_agent;
		return $this->userAgentMatch($user_agent);
	 ...
	 * @throws \Exception
	 */
	private function userAgentMatch($user_agent): string
	{
		return match ($user_agent) {
			'chrome' => 'Mozilla/5.0 ('.$this->getOS($user_agent).') AppleWebKit/'.$this->userAgentVers($user_agent),
			'firefox' => 'Mozilla/5.0 ('.$this->getOS($user_agent).') Gecko/'.$this->userAgentVers($user_agent),
			'mobile', 'android', 'iphone', 'ipad', 'ipod' => 'Mozilla/5.0 ('.$this->getMobileOS($user_agent).') AppleWebKit/'.$this->userAgentVers($user_agent),
			default => throw new Exception(__LINE__.': '.__METHOD__.' -> Unable to determine user agent to generate')
		};
	}
	 ...
	 * @throws \Random\RandomException
	 */
	private function userAgentVers($user_agent): string
	{
		return match ($user_agent) {
			'chrome' => (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)).'.'.random_int(1, 50).' (KHTML, like Gecko) Chrome/'.self::chromeVersion(['min' => 47, 'max' => 55]).' Safari/'.(random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)),
			'firefox' => (random_int(1, 100) > 30 ? '20100101' : '20130401').' Firefox/'.self::firefoxVersion(['min' => 45, 'max' => 74]),
			'explorer' => 'Mozilla / 5.0 (compatible; MSIE '.($int = random_int(7, 11)).'.0; '.$this->getOS('windows').' Trident / '.($int == 7 || $int == 8 ? '4' : ($int == 9 ? '5' : ($int == 10 ? '6' : '7'))).'.0)',
			'mobile', 'android', 'iphone', 'ipad', 'ipod' => (random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)).'.'.random_int(1, 50).' (KHTML, like Gecko) Chrome/'.self::chromeVersion(['min' => 47, 'max' => 55]).' Mobile Safari/'.(random_int(1, 100) > 50 ? random_int(533, 537) : random_int(600, 603)).'.'.random_int(0, 9)
		};
	}
	 ...
	 * @throws Exception
	 */
	public function randomUseragent(): string
	{
		$user_agent = dirname(__DIR__, 2).'/files/user_agent.txt';
		if(!file_exists($user_agent)) {
			return (new UserAgent())->generate('windows');
		}
		$user_list = file($user_agent);
		$random_ua = mt_rand(0, count($user_list) - 1);
		return trim($user_list[$random_ua]);
	}
